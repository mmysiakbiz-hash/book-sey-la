...
**Still missing vs Fresha/Booksy:** no-show/deposits + POS + tips (**Stripe**, J); **SMS** reminders/blasts (SMS provider); waitlist (needs a table); memberships/gift cards/packages (PWA mocks); per-staff working hours + individual calendars; client-facing loyalty progress on `/account`; tags/segments; intake forms; reschedule (owner move a booking); multi-location; real admin auth via the `admins` table (currently ADMIN_TOKEN). BI date-range filters.
