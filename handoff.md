...
**Fresha/Booksy parity — more added (all on pre-existing tables):**
- **Time off / blocked time**: `/panel Bookings → Block time` (`time_off` table, RLS owns_studio); `/api/book` rejects overlaps via `is_blocked()` definer rpc. `getTimeOff/addTimeOff/deleteTimeOff`.
- **Loyalty stamp cards**: `/panel Clients → Loyalty` (`loyalty_programs`, public read / owns_studio write); per-client stamps + Redeem (`loyalty_redemptions`); venue page shows the offer pill. `getLoyalty/saveLoyalty/redeemLoyalty`.
- **Marketing email blast**: `/panel Clients → Message clients` → `/api/marketing` (owner session; recipients = own studio's client emails from bookings; Brevo). `marketingEmail` template, `sendMarketing`.

**Still missing vs Fresha/Booksy:** no-show/deposits + POS + tips (**Stripe**, J); **SMS** reminders/blasts (SMS provider); gift cards; client-facing loyalty progress on `/account`; intake forms; multi-location. (Waitlist, tags/segments, memberships/packages, per-staff hours + individual calendars, real admin auth, BI date filters, reschedule — all now DONE, see below.)

**Fresha/Booksy parity — latest batches (all verified on live DB):**
- **Client tags/segments**: `client_notes.tags text[]`; tag chips + editor in the Clients CRM (`saveClientTags`); marketing blast can target a single tag (`/api/marketing` `?tag`, `MarketingComposer` segment selector).
- **Real admin login**: `/admin` signs in with a Supabase magic link — `/api/admin authorize()` accepts either `ADMIN_TOKEN` (break-glass) OR a session whose user is a row in `admins`; first login by an `ADMIN_EMAILS`-listed address auto-enrols. Email-first gate + sign-out; token behind a disclosure. (`ADMIN_EMAILS` env documented in `.env.example`.)
- **BI date-range filters**: `admin_bi(p_from, p_to)` windows every time-based metric + the daily chart (studio counts & MRR stay current); Overview has All / 7d / 30d / 90d selector.
- **Memberships & packages (display offer, no payments)**: new `packages` table (RLS mirrors services); owner "Packages" wizard step (`savePackages`); "Memberships & packages" cards on the studio page. Redeemed in-studio.
- **Non-destructive team save (bugfix)**: old `saveStaff` deleted+reinserted staff, which SET NULL'd every past booking's `staff_id` and cascade-deleted time-off on each edit. Now upserts by id + deactivates removed members (stable ids preserve history).
- **Opening-hours enforcement + per-staff hours**: `business_hours.staff_id` (null = studio default) + `within_hours(sid, staff, p)` definer fn (per-staff wins, else studio default, no-hours = open, Mahé tz); `/api/book` rejects `outside_hours`; per-person "Individual working hours" editor in the Team step (`saveStaffHours`). `saveHours` now only rewrites the studio-default rows.
- **Individual calendars**: agenda has an "All staff" / per-person filter.
