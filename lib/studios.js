...
function mapStudioDetail(r) {
  ...
  return {
    ...
    whatsapp: r.whatsapp || "",
    team: (Array.isArray(r.staff) ? r.staff : []).filter((s) => s.active !== false).map((s) => ({ name: s.name, role: s.role || "" })),
...
export async function getStudioBySlug(slug) {
  ...
  try {
    const { data, error } = await supabase
      .from("studios")
      .select(
        "id,slug,name,category,island,address,bio,tagline,whatsapp,socials,photos,google_rating,google_review_count,google_reviews,status,services(id,name,duration_min,price_eur,category,sort,active),staff(name,role,active),business_hours(day_of_week,open_min,close_min),reviews(rating,text,guest_name,created_at)"
      )
      .eq("slug", slug)
      .in("status", ["active", "verified"])
      .maybeSingle();
