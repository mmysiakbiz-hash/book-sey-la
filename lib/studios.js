...
function mapStudioDetail(r) {
  ...
  return {
    ...
    team: (Array.isArray(r.staff) ? r.staff : []).filter((s) => s.active !== false).map((s) => ({
      id: s.id, name: s.name, role: s.role || "",
      img: s.photo_url || "",
      services: Array.isArray(s.services) ? s.services : [], // service names this person offers ([] = all)
    })),
