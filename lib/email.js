...
export async function sendBrevoEmail({ to, toName, subject, html, replyTo }) {
  ...
  try {
    const res = await fetch(BREVO_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        // A Reply-To (e.g. the contact form's sender) so replies reach the person,
        // not the no-reply sender address.
        ...(replyTo && replyTo.email ? { replyTo: { email: replyTo.email, name: replyTo.name || undefined } } : {}),
        subject,
        htmlContent: html,
      }),
    });
...
export function billingBlockedEmail({ studioName }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F5F4F1;padding:28px 16px"><tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#FFFFFF;border:1px solid #E9E5E0;border-radius:16px;overflow:hidden">
      <tr><td style="padding:24px 26px 8px"><div style="font-size:17px;color:#23201E"><a href="https://book.sey.la" style="color:inherit;text-decoration:none"><b>sey.la</b> <span style="color:#9A938D">|</span> book</a></div></td></tr>
    ...
    <p style="max-width:480px;margin:16px auto 0;color:#9A938D;font-size:12px;text-align:center"><a href="https://book.sey.la" style="color:inherit;text-decoration:none">sey.la | book</a> · beauty &amp; wellness across the Seychelles</p>
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#FFFFFF;border:1px solid #E9E5E0;border-radius:16px;overflow:hidden">
        <tr><td style="padding:24px 26px 8px">
          <div style="font-size:17px;color:#23201E"><a href="https://book.sey.la" style="color:inherit;text-decoration:none"><b>sey.la</b> <span style="color:#9A938D">|</span> book</a></div>
        </td></tr>
        <tr><td style="padding:8px 26px 4px">
          <h1 style="margin:0;font-size:22px;color:#23201E">Sign in to <a href="https://book.sey.la" style="color:inherit;text-decoration:none">sey.la | book</a></h1>
...
// Sent when someone joins a class that's already full — they're on the waitlist
// and we'll email them if a spot frees up.
export function classWaitlistEmail({ studioName, className, whenText }) {
  const subject = `You're on the waitlist · ${className || "class"}${studioName ? " · " + studioName : ""}`;
  const row = (l, v) => v ? `<tr><td style="padding:6px 0;color:#6E6863;font-size:14px">${l}</td><td style="padding:6px 0;color:#23201E;font-size:14px;font-weight:600;text-align:right">${v}</td></tr>` : "";
  const html = `<!doctype html><html><body style="margin:0;background:#F5F4F1;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F5F4F1;padding:28px 16px"><tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#FFFFFF;border:1px solid #E9E5E0;border-radius:16px;overflow:hidden">
      <tr><td style="padding:24px 26px 8px"><div style="font-size:17px;color:#23201E"><a href="https://book.sey.la" style="color:inherit;text-decoration:none"><b>sey.la</b> <span style="color:#9A938D">|</span> book</a></div></td></tr>
      <tr><td style="padding:8px 26px 4px"><h1 style="margin:0;font-size:22px;color:#23201E">You're on the waitlist</h1><p style="margin:8px 0 0;color:#47423F;font-size:15px;line-height:1.5">This class is full right now. We've saved your place in line — if a spot opens up, we'll email you straight away.</p></td></tr>
      <tr><td style="padding:16px 26px 4px"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #E9E5E0">
        ${row("Studio", studioName)}${row("Class", className)}${row("When", whenText)}${row("Status", "Waitlisted")}
      </table></td></tr>
      <tr><td style="padding:18px 26px 26px"><p style="margin:0;color:#6E6863;font-size:13px;line-height:1.5">No spot is held yet — you'll only be booked in once we confirm by email.</p></td></tr>
    </table>
    <p style="max-width:480px;margin:16px auto 0;color:#9A938D;font-size:12px;text-align:center"><a href="https://book.sey.la" style="color:inherit;text-decoration:none">sey.la | book</a> · beauty &amp; wellness across the Seychelles</p>
  </td></tr></table></body></html>`;
  return { subject, html };
...
// Studio → clients marketing message (owner composes body; we brand it).
export function marketingEmail({ studioName, message, bookUrl }) {
  const safe = String(message || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>");
  const subject = `${studioName || "Your studio"} on sey.la`;
  const html = `<!doctype html><html><body style="margin:0;background:#F5F4F1;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F5F4F1;padding:28px 16px"><tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;background:#FFFFFF;border:1px solid #E9E5E0;border-radius:16px;overflow:hidden">
      <tr><td style="padding:24px 26px 8px"><div style="font-size:17px;color:#23201E"><b>${studioName || "sey.la | book"}</b></div></td></tr>
      <tr><td style="padding:8px 26px 8px;color:#33302E;font-size:15px;line-height:1.6">${safe}</td></tr>
      ${bookUrl ? `<tr><td style="padding:8px 26px 24px"><a href="${bookUrl}" style="display:inline-block;background:#23201E;color:#fff;text-decoration:none;font-size:15px;font-weight:600;padding:12px 22px;border-radius:999px">Book now</a></td></tr>` : ""}
    </table>
    <p style="max-width:480px;margin:16px auto 0;color:#9A938D;font-size:12px;text-align:center">Sent via <a href="https://book.sey.la" style="color:inherit;text-decoration:none">sey.la | book</a> · you're receiving this because you booked here.</p>
  </td></tr></table></body></html>`;
  return { subject, html };
}
