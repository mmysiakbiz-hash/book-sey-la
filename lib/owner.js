...
export async function saveStaff(studioId, items) {
  ...
  for (let i = 0; i < clean.length; i++) {
    const s = clean[i];
    const base = { name: s.name.trim(), role: s.role ? s.role.trim() : null, color: s.color || palette[i % palette.length], active: true };
    if (s.id) {
