...
function parseRange(during) {
  ...
  // Postgres emits the space form with a bare-hour offset ("2026-07-21 11:00:00+00").
  // Parse it as-is — replacing the space with "T" makes the "+00" offset unparseable.
  const s = new Date(m[1].trim());
  const e = new Date(m[2].trim());
  return {
...
export async function getMyBookings() {
  ...
  const { data, error } = await supabase
    .from("bookings")
    .select("id, studio_id, service_id, staff_id, during, status, price_eur, service_name, studios(name, slug), services(name), staff(name)")
  ...
  return (data || []).map((b) => {
    ...
    return {
      ...
      serviceName: b.service_name || b.services?.name || "",
      staffId: b.staff_id,
...
export async function rescheduleMyBooking(id, studioId, startsAt, durationMin, staffId) {
  ...
  if (error) {
    // The no_overlap EXCLUDE constraint (staff_id + during, SQLSTATE 23P01) rejects
    // moving into a slot another booking already holds — surface it as a clear,
    // pickable message instead of the raw Postgres error.
    if (error.code === "23P01" || /no_overlap|exclusion constraint/i.test(error.message || "")) {
