...
// - Then 500 SCR per user (the owner is the first user, +500 for each staff
//   member) per cycle — i.e. 500 × max(1, staff).
...
export function billingStatus(studio, staffCount) {
  ...
  const users = Math.max(1, staffCount || 0); // owner is the first user
  const amount = SCR_PER_USER * users;
